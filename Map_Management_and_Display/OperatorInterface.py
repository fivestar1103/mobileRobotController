...
class OperatorInterface:
    ...
    def create_widgets(self):
        ...
        second_row = tk.Frame(self.__master, width=300, height=100)
    ...
    def create_input_section(self, parent, label_text, command, display=False):
        ...
        canvas = tk.Canvas(frame, width=40, height=40, bg=self.color2, highlightthickness=0)
        canvas.pack(pady=8)
        circle = canvas.create_oval(5, 5, 35, 35, fill=self.color3, outline=self.color3)
        text_id = canvas.create_text(20, 20, text="OK", fill=self.color4)

        def on_click(event):
            command(col_entry.get(), row_entry.get(), frame)
        ...
        if display:
            ...
            display_area = tk.Listbox(list_frame, height=10, width=10,font=("Serif", 14), bg=self.color1, fg=self.color2)
